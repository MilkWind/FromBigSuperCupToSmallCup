...
function handleDeleteTier() {
  if (confirm(`确定要删除等级"${props.tier.name}"吗？`)) {
...
<template>
  <div class="border border-gray-600 rounded-md overflow-hidden mb-1.5 bg-gray-800 flex min-h-[60px]">
    <div class="flex items-center justify-center px-3 py-2 min-w-[120px] max-w-[120px] font-semibold text-sm relative flex-shrink-0 group" :style="tierStyle">
      <div class="text-center w-full">{{ tier.name }}</div>
      <div v-if="canEdit" class="absolute top-1 right-1 flex gap-0.5 opacity-0 transition-opacity duration-200 group-hover:opacity-100">
        <button
